...
`;console.info("%c BERM-THERMAL-FLOW-CARD %c 0.1.0 ","color: white; font-weight: bold; background: #039be5","color: #039be5; font-weight: bold; background: white");let ee=class extends nt{constructor(){super(...arguments),this._loading=!0}static async getConfigElement(){return document.createElement("berm-thermal-flow-card-editor")}static getStubConfig(){return{type:`custom:${pt}`,entities:{outside:{temperature:"sensor.outdoor_temperature"},fans:[],rooms:[]}}}static get styles(){return te}setConfig(t){try{this._config=function(t){if(!t)throw new Error(Dt);if(!t.entities)throw new Error(Dt);if(!t.entities.outside||!t.entities.outside.temperature)throw new Error(Bt);return Array.isArray(t.entities.fans)||(t.entities.fans=[]),Array.isArray(t.entities.rooms)||(t.entities.rooms=[]),t.entities.fans.forEach((t,e)=>{if(!t.name)throw new Error(`Fan at index ${e} is missing 'name'`);if(!t.speed)throw new Error(`Fan '${t.name}' is missing 'speed' entity`);t.power_map||(t.power_map=ut)}),t.entities.rooms.forEach((e,i)=>{if(!e.name)throw new Error(`Room at index ${i} is missing 'name'`);if(!e.temperature)throw new Error(`Room '${e.name}' is missing 'temperature' entity`);if(void 0!==e.fan_index&&null!==e.fan_index&&(e.fan_index<0||e.fan_index>=t.entities.fans.length))throw new Error(`Room '${e.name}' has invalid fan_index ${e.fan_index}. Must be between 0 and ${t.entities.fans.length-1}`)}),{type:"custom:berm-thermal-flow-card",entities:{outside:t.entities.outside,fans:t.entities.fans,rooms:t.entities.rooms,greenhouse:t.entities.greenhouse},colors:Qt(ft,t.colors||{}),temperature_thresholds:Qt(mt,t.temperature_thresholds||{}),animation:Qt($t,t.animation||{}),display:Qt(_t,t.display||{})}}(t),this._error=void 0}catch(t){this._error=t instanceof Error?t.message:"Unknown error",console.error("Berm Thermal Flow Card configuration error:",t)}}getCardSize(){return 12}shouldUpdate(t){return!!this._config&&(!!t.has("_config")||(!t.has("hass")||this._updateCardState()))}updated(t){super.updated(t),t.has("hass")&&this.hass&&(this._loading&&(this._loading=!1),this._cardState||(this._updateCardState(),this.requestUpdate()))}_updateCardState(){if(!this.hass||!this._config)return!1;try{const t=this._computeCardState();return JSON.stringify(t)!==JSON.stringify(this._cardState)&&(this._cardState=t,!0)}catch(t){return console.error("Error updating card state:",t),!1}}_computeCardState(){const{entities:t,colors:e,temperature_thresholds:i}=this._config,o=Vt(this.hass,t.outside.temperature,70),s={temperature:o,rate:t.outside.rate?Vt(this.hass,t.outside.rate,0):void 0,color:It(o,i,e)},r=t.fans.map(t=>{const e=Math.round(Vt(this.hass,t.speed,0)),i=function(t,e=ut){const i=Math.round(t).toString();return e[i]??ut[i]??0}(e,t.power_map||ut);return{name:t.name,speed:Math.max(0,Math.min(10,e)),power:i,offline:t.offline||!Gt(this.hass,t.speed),icon:t.icon||gt}}),n=t.rooms.map(t=>{const o=Vt(this.hass,t.temperature,70),s=t.delta?Vt(this.hass,t.delta,0):void 0,r=t.desired_temp?Vt(this.hass,t.desired_temp,70):void 0;return{name:t.name,temperature:o,delta:s,desired_temp:r,fan_index:t.fan_index,color:It(o,i,e),icon:t.icon||yt}});let a;if(t.greenhouse?.enabled&&t.greenhouse.temperature){const o=Vt(this.hass,t.greenhouse.temperature,70);a={temperature:o,delta:t.greenhouse.delta?Vt(this.hass,t.greenhouse.delta,0):void 0,color:It(o,i,e)}}return{outside:s,fans:r,rooms:n,greenhouse:a}}_generateFlowLines(){if(!this._cardState||!this._config)return[];const t=[],{fans:e,rooms:i}=this._cardState,{animation:o}=this._config,s=Ct,r=Ot,n=St,a=xt/2;return e.forEach((i,r)=>{const d=this._getFanX(r,e.length),c=i.speed>0&&!i.offline;t.push({id:`outside-fan-${r}`,path:Zt(a,n+wt,d,s-bt),from:{x:a,y:n},to:{x:d,y:s},speed:i.speed,animationDuration:c?Xt(i.speed,o):0,color:this._cardState.outside.color,active:c})}),i.forEach((n,a)=>{if(void 0!==n.fan_index&&null!==n.fan_index&&n.fan_index>=0&&n.fan_index<e.length){const d=e[n.fan_index],c=this._getFanX(n.fan_index,e.length),l=this._getRoomX(a,i.length),h=d.speed>0&&!d.offline;t.push({id:`fan${n.fan_index}-room${a}`,path:Zt(c,s+bt,l,r-At),from:{x:c,y:s},to:{x:l,y:r},speed:d.speed,animationDuration:h?Xt(d.speed,o):0,color:n.color,active:h})}}),t}_getFanX(t,e){return kt+(xt-2*kt)/(e+1)*(t+1)}_getRoomX(t,e){return kt+(xt-2*kt)/(e+1)*(t+1)}render(){return this._error?this._renderError(this._error):!this._loading&&this._config&&this._cardState?B`
      <ha-card>
        <div class="card-content">
          ${this._renderSVG()}
        </div>
      </ha-card>
    `:this._renderLoading()}_renderError(t){return B`
      <ha-card>
        <div class="error">
    ...
    `}_renderSVG(){const t=this._generateFlowLines();return B`
      <svg viewBox="0 0 ${xt} ${vt}" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Fan nodes -->
        <g class="fan-nodes">
          ${this._cardState.fans.map((t,e)=>this._renderFanNode(t,e))}
        </g>

        <!-- Room nodes -->
        <g class="room-nodes">
          ${this._cardState.rooms.map((t,e)=>this._renderRoomNode(t,e))}
        </g>
    ...
    `}_renderFlowDots(t){const{animation:e}=this._config,i=e?.dots_per_line||3,o=e?.dot_size||6;return B`
      ${Array.from({length:i},(e,s)=>{const r=t.animationDuration/i*s;return B`
          ...
          />
        `})}
    `}_renderOutsideNode(){const{outside:t}=this._cardState,{display:e}=this._config,i=xt/2,o=St,s=wt;return B`
      <g class="node ${Nt}" data-entity="${this._config.entities.outside.temperature}">
        <circle cx="${i}" cy="${o}" r="${s}" fill="${t.color}" fill-opacity="0.2" />

        <!-- Temperature -->
        <text x="${i}" y="${o-5}" class="${Lt}">
          ${Jt(t.temperature,e?.temperature_unit)}
        </text>

        <!-- Rate of change (if available and enabled) -->
        ${e?.show_rate_of_change&&void 0!==t.rate?B`
          <text x="${i}" y="${o+15}" class="${jt}">
            ${Kt(t.rate)}
          </text>
        `:""}

        <!-- Label -->
        <text x="${i}" y="${o+s+20}" class="${zt}">Outside</text>
      </g>
    `}_renderFanNode(t,e){const{display:i}=this._config,o=this._getFanX(e,this._cardState.fans.length),s=Ct,r=bt;return B`
      <g class="node ${Tt} ${t.offline?"offline":""}" data-fan-index="${e}">
        <circle cx="${o}" cy="${s}" r="${r}" fill="#808080" fill-opacity="${t.offline?.1:.3}" />

        <!-- Fan speed -->
        <text x="${o}" y="${s-10}" class="${Lt}">
          ${t.offline?"OFF":t.speed}
        </text>

        <!-- Power consumption (if enabled) -->
        ${i?.show_power&&!t.offline?B`
          <text x="${o}" y="${s+10}" class="${jt}">
            ${n=t.power,0===n?"OFF":n<1e3?`${n}W`:`${(n/1e3).toFixed(2)}kW`}
          </text>
        `:""}

        <!-- Label -->
        <text x="${o}" y="${s+r+20}" class="${zt}">${t.name}</text>

        ${t.offline?B`
          <text x="${o}" y="${s+r+35}" class="offline-text">OFFLINE</text>
        `:""}
      </g>
    `;var n}_renderRoomNode(t,e){const{display:i}=this._config,o=this._getRoomX(e,this._cardState.rooms.length),s=Ot,r=At,n=void 0===t.fan_index||null===t.fan_index;return B`
      <g class="node ${Ft} ${n?"static":""}" data-room-index="${e}">
        <circle cx="${o}" cy="${s}" r="${r}" fill="${t.color}" fill-opacity="0.3" />

        <!-- Temperature -->
        <text x="${o}" y="${s-5}" class="${Lt}">
          ${Jt(t.temperature,i?.temperature_unit)}
        </text>

        <!-- Rate of change (if available and enabled) -->
        ${i?.show_rate_of_change&&void 0!==t.delta?B`
          <text x="${o}" y="${s+15}" class="${jt}">
            ${Kt(t.delta)}
          </text>
        `:""}
    ...
    `}_renderGreenhouseNode(){const{greenhouse:t}=this._cardState;if(!t)return B``;const{display:e}=this._config,i=xt/2,o=Pt,s=Et;return B`
      <g class="node ${Ht}">
