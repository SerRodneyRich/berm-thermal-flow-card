...
`;console.info("%c BERM-THERMAL-FLOW-CARD %c 0.1.0 ","color: white; font-weight: bold; background: #039be5","color: #039be5; font-weight: bold; background: white");let oe=class extends ct{static getConfigElement(){}static getStubConfig(){return{type:`custom:${ft}`,entities:{outside:{temperature:"sensor.outdoor_temperature"},fans:[],rooms:[]}}}static get styles(){return ie}setConfig(t){try{this._config=function(t){if(!t)throw new Error(It);if(!t.entities)throw new Error(It);if(!t.entities.outside||!t.entities.outside.temperature)throw new Error(Vt);const e=Array.isArray(t.entities.fans)?t.entities.fans:[],i=Array.isArray(t.entities.rooms)?t.entities.rooms:[],o=e.map((t,e)=>{if(!t.name)throw new Error(`Fan at index ${e} is missing 'name'`);if(!t.speed)throw new Error(`Fan '${t.name}' is missing 'speed' entity`);return{...t,power_map:t.power_map||mt}});return i.forEach((t,e)=>{if(!t.name)throw new Error(`Room at index ${e} is missing 'name'`);if(!t.temperature)throw new Error(`Room '${t.name}' is missing 'temperature' entity`);if(void 0!==t.fan_index&&null!==t.fan_index&&(t.fan_index<0||t.fan_index>=o.length))throw new Error(`Room '${t.name}' has invalid fan_index ${t.fan_index}. Must be between 0 and ${o.length-1}`)}),{type:"custom:berm-thermal-flow-card",entities:{outside:t.entities.outside,fans:o,rooms:i,greenhouse:t.entities.greenhouse},colors:te($t,t.colors||{}),temperature_thresholds:te(_t,t.temperature_thresholds||{}),animation:te(gt,t.animation||{}),display:te(yt,t.display||{})}}(t),this._error=void 0}catch(t){this._error=t instanceof Error?t.message:"Unknown error",console.error("Berm Thermal Flow Card configuration error:",t)}}getCardSize(){return 12}shouldUpdate(t){return!(!this._config||!this.hass)}_computeCardState(){const{entities:t,colors:e,temperature_thresholds:i}=this._config,o=Xt(this.hass,t.outside.temperature,70),s={temperature:o,rate:t.outside.rate?Xt(this.hass,t.outside.rate,0):void 0,color:Wt(o,i,e)},r=t.fans.map(t=>{const e=Math.round(Xt(this.hass,t.speed,0)),i=function(t,e=mt){const i=Math.round(t).toString();return e[i]??mt[i]??0}(e,t.power_map||mt);return{name:t.name,speed:Math.max(0,Math.min(10,e)),power:i,offline:t.offline||!Jt(this.hass,t.speed),icon:t.icon||xt}}),n=t.rooms.map(t=>{const o=Xt(this.hass,t.temperature,70),s=t.delta?Xt(this.hass,t.delta,0):void 0,r=t.desired_temp?Xt(this.hass,t.desired_temp,70):void 0;return{name:t.name,temperature:o,delta:s,desired_temp:r,fan_index:t.fan_index,color:Wt(o,i,e),icon:t.icon||vt}});let a;if(t.greenhouse?.enabled&&t.greenhouse.temperature){const o=Xt(this.hass,t.greenhouse.temperature,70);a={temperature:o,delta:t.greenhouse.delta?Xt(this.hass,t.greenhouse.delta,0):void 0,color:Wt(o,i,e)}}return{outside:s,fans:r,rooms:n,greenhouse:a}}_generateFlowLines(t){if(!this._config)return[];const e=[],{fans:i,rooms:o}=t,{animation:s}=this._config,r=kt,n=Ot,a=Pt,c=wt/2;return i.forEach((o,n)=>{const l=this._getFanX(n,i.length),d=o.speed>0&&!o.offline;e.push({id:`outside-fan-${n}`,path:Yt(c,a+At,l,r-Et),from:{x:c,y:a},to:{x:l,y:r},speed:o.speed,animationDuration:d?Kt(o.speed,s):0,color:t.outside.color,active:d})}),o.forEach((t,a)=>{if(void 0!==t.fan_index&&null!==t.fan_index&&t.fan_index>=0&&t.fan_index<i.length){const c=i[t.fan_index],l=this._getFanX(t.fan_index,i.length),d=this._getRoomX(a,o.length),h=c.speed>0&&!c.offline;e.push({id:`fan${t.fan_index}-room${a}`,path:Yt(l,r+Et,d,n-St),from:{x:l,y:r},to:{x:d,y:n},speed:c.speed,animationDuration:h?Kt(c.speed,s):0,color:t.color,active:h})}}),e}_getFanX(t,e){return Rt+(wt-2*Rt)/(e+1)*(t+1)}_getRoomX(t,e){return Rt+(wt-2*Rt)/(e+1)*(t+1)}render(){if(!this._config||!this.hass)return I``;if(this._error)return I`
        <ha-card>
          <div class="error">
      ...
      `;const t=this._computeCardState();return I`
      <ha-card>
    ...
    `}_renderFlowDots(t){const{animation:e}=this._config,i=e?.dots_per_line||3,o=e?.dot_size||6;return V`
      ${Array.from({length:i},(e,s)=>{const r=t.animationDuration/i*s;return V`
          <circle
            class="flow-dot"
            r="${o/2}"
            fill="${t.color}"
        ...
        `})}
    `}_renderOutsideNode(t){const{outside:e}=t,{display:i}=this._config,o=wt/2,s=Pt,r=At;return V`
      <g class="node ${Ft}" data-entity="${this._config.entities.outside.temperature}">
        <circle cx="${o}" cy="${s}" r="${r}" fill="${e.color}" fill-opacity="0.2" />

        <!-- Temperature -->
        <text x="${o}" y="${s-5}" class="${Dt}">
          ${Zt(e.temperature,i?.temperature_unit)}
        </text>

        <!-- Rate of change (if available and enabled) -->
        ${i?.show_rate_of_change&&void 0!==e.rate?V`
          <text x="${o}" y="${s+15}" class="${Bt}">
            ${Qt(e.rate)}
          </text>
        `:""}

        <!-- Label -->
        <text x="${o}" y="${s+r+20}" class="${Lt}">Outside</text>
      </g>
    `}_renderFanNode(t,e,i){const{display:o}=this._config,s=this._getFanX(e,i),r=kt,n=Et;return V`
      <g class="node ${Ht} ${t.offline?"offline":""}" data-fan-index="${e}">
        <circle cx="${s}" cy="${r}" r="${n}" fill="#808080" fill-opacity="${t.offline?.1:.3}" />

        <!-- Fan speed -->
        <text x="${s}" y="${r-10}" class="${Dt}">
          ${t.offline?"OFF":t.speed}
        </text>

        <!-- Power consumption (if enabled) -->
        ${o?.show_power&&!t.offline?V`
          <text x="${s}" y="${r+10}" class="${Bt}">
            ${a=t.power,0===a?"OFF":a<1e3?`${a}W`:`${(a/1e3).toFixed(2)}kW`}
          </text>
        `:""}

        <!-- Label -->
        <text x="${s}" y="${r+n+20}" class="${Lt}">${t.name}</text>

        ${t.offline?V`
          <text x="${s}" y="${r+n+35}" class="offline-text">OFFLINE</text>
        `:""}
      </g>
    `;var a}_renderRoomNode(t,e,i){const{display:o}=this._config,s=this._getRoomX(e,i),r=Ot,n=St,a=void 0===t.fan_index||null===t.fan_index;return V`
      <g class="node ${zt} ${a?"static":""}" data-room-index="${e}">
        <circle cx="${s}" cy="${r}" r="${n}" fill="${t.color}" fill-opacity="0.3" />

        <!-- Temperature -->
        <text x="${s}" y="${r-5}" class="${Dt}">
          ${Zt(t.temperature,o?.temperature_unit)}
        </text>

        <!-- Rate of change (if available and enabled) -->
        ${o?.show_rate_of_change&&void 0!==t.delta?V`
          <text x="${s}" y="${r+15}" class="${Bt}">
            ${Qt(t.delta)}
          </text>
        `:""}
