...
`;console.info("%c BERM-THERMAL-FLOW-CARD %c 0.1.0 ","color: white; font-weight: bold; background: #039be5","color: #039be5; font-weight: bold; background: white");let se=class extends ct{static getConfigElement(){}static getStubConfig(){return{type:`custom:${ft}`,entities:{outside:{temperature:"sensor.outdoor_temperature"},fans:[],rooms:[]}}}static get styles(){return ie}setConfig(t){try{this._config=function(t){if(!t)throw new Error(Vt);if(!t.entities)throw new Error(Vt);if(!t.entities.outside||!t.entities.outside.temperature)throw new Error(qt);const e=Array.isArray(t.entities.fans)?t.entities.fans:[],o=Array.isArray(t.entities.rooms)?t.entities.rooms:[],i=e.map((t,e)=>{if(!t.name)throw new Error(`Fan at index ${e} is missing 'name'`);if(!t.speed)throw new Error(`Fan '${t.name}' is missing 'speed' entity`);return{...t,power_map:t.power_map||mt}});return o.forEach((t,e)=>{if(!t.name)throw new Error(`Room at index ${e} is missing 'name'`);if(!t.temperature)throw new Error(`Room '${t.name}' is missing 'temperature' entity`);if(void 0!==t.fan_index&&null!==t.fan_index&&(t.fan_index<0||t.fan_index>=i.length))throw new Error(`Room '${t.name}' has invalid fan_index ${t.fan_index}. Must be between 0 and ${i.length-1}`)}),{type:"custom:berm-thermal-flow-card",entities:{outside:t.entities.outside,fans:i,rooms:o,greenhouse:t.entities.greenhouse},colors:ee($t,t.colors||{}),temperature_thresholds:ee(_t,t.temperature_thresholds||{}),animation:ee(gt,t.animation||{}),display:ee(yt,t.display||{})}}(t),this._error=void 0}catch(t){this._error=t instanceof Error?t.message:"Unknown error",console.error("Berm Thermal Flow Card configuration error:",t)}}getCardSize(){return 12}shouldUpdate(t){return!(!this._config||!this.hass)}_computeCardState(){const{entities:t,colors:e,temperature_thresholds:o}=this._config,i=Jt(this.hass,t.outside.temperature,70),s={temperature:i,rate:t.outside.rate?Jt(this.hass,t.outside.rate,0):void 0,color:Gt(i,o,e)},n=t.fans.map(t=>{const e=Math.round(Jt(this.hass,t.speed,0)),o=function(t,e=mt){const o=Math.round(t).toString();return e[o]??mt[o]??0}(e,t.power_map||mt);return{name:t.name,speed:Math.max(0,Math.min(10,e)),power:o,offline:t.offline||!Kt(this.hass,t.speed),icon:t.icon||xt}}),r=t.rooms.map(t=>{const i=Jt(this.hass,t.temperature,70),s=t.delta?Jt(this.hass,t.delta,0):void 0,n=t.desired_temp?Jt(this.hass,t.desired_temp,70):void 0;return{name:t.name,temperature:i,delta:s,desired_temp:n,fan_index:t.fan_index,color:Gt(i,o,e),icon:t.icon||vt}});let a;if(t.greenhouse?.enabled&&t.greenhouse.temperature){const i=Jt(this.hass,t.greenhouse.temperature,70);a={temperature:i,delta:t.greenhouse.delta?Jt(this.hass,t.greenhouse.delta,0):void 0,color:Gt(i,o,e)}}return{outside:s,fans:n,rooms:r,greenhouse:a}}_generateFlowLines(t){if(!this._config)return[];const e=[],{fans:o,rooms:i}=t,{animation:s}=this._config,n=Mt,r=Ot,a=Pt,c=wt/2;return o.forEach((i,r)=>{const l=this._getFanX(r,o.length),d=i.speed>0&&!i.offline;e.push({id:`outside-fan-${r}`,path:te(c,a+At,l,n-Et),from:{x:c,y:a},to:{x:l,y:n},speed:i.speed,animationDuration:d?Yt(i.speed,s):0,color:t.outside.color,active:d})}),i.forEach((t,a)=>{if(void 0!==t.fan_index&&null!==t.fan_index&&t.fan_index>=0&&t.fan_index<o.length){const c=o[t.fan_index],l=this._getFanX(t.fan_index,o.length),d=this._getRoomX(a,i.length),h=c.speed>0&&!c.offline;e.push({id:`fan${t.fan_index}-room${a}`,path:te(l,n+Et,d,r-St),from:{x:l,y:n},to:{x:d,y:r},speed:c.speed,animationDuration:h?Yt(c.speed,s):0,color:t.color,active:h})}}),e}_getFanX(t,e){return Ut+(wt-2*Ut)/(e+1)*(t+1)}_getRoomX(t,e){return Ut+(wt-2*Ut)/4*(t%3+1)}_getRoomY(t){const e=Math.floor(t/3);return Ot+e*kt}render(){if(!this._config||!this.hass)return B``;if(this._error)return B`
        <ha-card>
          <div class="error">
    ...
    `}_renderOutsideNode(t){const{outside:e}=t,{display:o}=this._config,i=wt/2,s=Pt,n=At;return V`
      <g class="node ${zt}" data-entity="${this._config.entities.outside.temperature}">
        <circle cx="${i}" cy="${s}" r="${n}" fill="${e.color}" fill-opacity="0.15" />

        <!-- Temperature (top) -->
        <text x="${i}" y="${s-n+42}" class="${It}">
          ${Zt(e.temperature,o?.temperature_unit)}
        </text>

        <!-- Icon (center) -->
        <text x="${i}" y="${s+8}" class="node-icon">☁️</text>

        <!-- Rate of change (bottom) -->
        ${o?.show_rate_of_change&&void 0!==e.rate?V`
          <text x="${i}" y="${s+n-30}" class="${Bt}">
            ${Qt(e.rate)}
          </text>
        `:""}

        <!-- Label (below circle) -->
        <text x="${i}" y="${s+n+32}" class="${Dt}">Outside</text>
      </g>
    `}_renderFanNode(t,e,o){const i=this._getFanX(e,o),s=Mt,n=Et;return V`
      <g class="node ${Ht} ${t.offline?"offline":""}" data-fan-index="${e}">
        ...
        <text x="${i}" y="${s}" class="fan-number">
          ${t.offline?"OFF":t.speed}
        </text>

        <!-- Label (below circle) -->
        <text x="${i}" y="${s+n+32}" class="${Dt}">${t.name}</text>

        ${t.offline?V`
          <text x="${i}" y="${s+n+52}" class="offline-text">OFFLINE</text>
        `:""}
      </g>
    `}_renderRoomNode(t,e,o){const{display:i}=this._config,s=this._getRoomX(e,o),n=this._getRoomY(e),r=St,a=void 0===t.fan_index||null===t.fan_index;return V`
      <g class="node ${jt} ${a?"static":""}" data-room-index="${e}">
        <circle cx="${s}" cy="${n}" r="${r}" fill="${t.color}" fill-opacity="0.2" />

        <!-- Temperature (top) -->
        <text x="${s}" y="${n-r+42}" class="${It}">
          ${Zt(t.temperature,i?.temperature_unit)}
        </text>

        <!-- Icon (center) -->
        <text x="${s}" y="${n+8}" class="node-icon">🏠</text>

        <!-- Rate of change (bottom) -->
        ${i?.show_rate_of_change&&void 0!==t.delta?V`
          <text x="${s}" y="${n+r-30}" class="${Bt}">
            ${Qt(t.delta)}
          </text>
        `:""}

        <!-- Label (below circle) -->
        <text x="${s}" y="${n+r+32}" class="${Dt}">${t.name}</text>
      </g>
    `}_renderGreenhouseNode(t){const{display:e}=this._config,o=wt/2,i=Rt,s=Ct;return V`
      <g class="node ${Lt}">
        <circle cx="${o}" cy="${i}" r="${s}" fill="${t.color}" fill-opacity="0.2" />

        <!-- Temperature (top) -->
        <text x="${o}" y="${i-s+42}" class="${It}">
          ${Zt(t.temperature,e?.temperature_unit)}
        </text>

        <!-- Icon (center) -->
        <text x="${o}" y="${i+8}" class="node-icon">🌿</text>

        <!-- Rate of change (bottom) -->
        ${e?.show_rate_of_change&&void 0!==t.delta?V`
          <text x="${o}" y="${i+s-30}" class="${Bt}">
            ${Qt(t.delta)}
          </text>
        `:""}

        <!-- Label (below circle, consistent with other nodes) -->
        <text x="${o}" y="${i+s+32}" class="${Dt}">Greenhouse</text>
      </g>
    `}};t([pt({attribute:!1})],se.prototype,"hass",void 0),t([ut()],se.prototype,"_config",void 0),t([ut()],se.prototype,"_error",void 0),se=t([(t=>(e,o)=>{void 0!==o?o.addInitializer(()=>{customElements.define(t,e)}):customElements.define(t,e)})(ft)],se),window.customCards=window.customCards||[],window.customCards.push({type:ft,name:"Berm Thermal Flow Card",description:"Visualize airflow from outside temperature through fans to rooms with thermal analysis"});export{se as BermThermalFlowCard};
